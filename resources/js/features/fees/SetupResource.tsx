...
export function SetupResource({
    resource, singular, extraFields = [], queryParams = {},
}: {
    resource: string;
    singular: string;
    ...
    const key = ['fees-setup', resource, queryParams];
    ...
    const { data: rows = [], isLoading } = useQuery({ queryKey: key, queryFn: () => listSetup(resource, queryParams) });
                                        ...
                                        <td key={f.name} className="px-5 py-3 text-muted">
                                            {f.name === 'fee_head_id'
                                                ...
                                                : f.type === 'checkbox'
                                                    ...
                                                    : f.type === 'select'
                                                        ? (f.options?.find((o) => o.value === String(r[f.name]))?.label ?? (r[f.name] as string) ?? '—')
                                                        : ((r[f.name] as string) || '—')}
                                        </td>
                                    ))}
                        ...
                        {f.type === 'checkbox' ? (
                            ...
                            <>
                                ...
                                <select
                                    ...
                                    onChange={(e) => set(f.name, f.name.endsWith('_id') ? Number(e.target.value) : e.target.value)}
