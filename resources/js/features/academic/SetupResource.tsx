...
export function SetupResource({
    ...
    const invalidate = () => qc.invalidateQueries({ queryKey: ['setup', resource] });
...
function defaults(fields: FieldDef[], row: SetupRow | null): Record<string, unknown> {
    ...
    for (const f of fields) out[f.name] = row ? (row[f.name] ?? (f.type === 'checkbox' ? false : '')) : (f.type === 'checkbox' ? false : f.type === 'number' ? 0 : '');
    return out;
}
