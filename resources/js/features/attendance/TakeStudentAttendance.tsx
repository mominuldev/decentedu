...
import { studentRoster, takeStudentAttendance, STATUSES, type AttendanceStatus } from './api';
...
export function TakeStudentAttendance() {
    ...
    useEffect(() => {
        ...
        for (const r of roster) next[r.student_id] = { status: r.status, remarks: r.remarks ?? '' };
        setEntries(next);
        setSaved(false);
    }, [roster]);

    const setStatus = (studentId: number, status: AttendanceStatus) => {
        setEntries((e) => ({ ...e, [studentId]: { ...e[studentId], status } }));
        setSaved(false);
    };
    const setRemarks = (studentId: number, remarks: string) => {
        setEntries((e) => ({ ...e, [studentId]: { ...e[studentId], remarks } }));
        setSaved(false);
    };
    ...
    const save = useMutation({
        mutationFn: () => takeStudentAttendance(
            classConfigId,
            date,
            roster.map((r) => ({ student_id: r.student_id, status: entries[r.student_id].status, remarks: entries[r.student_id].remarks || null })),
        ),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['student-roster', classConfigId, date] });
            setSaved(true);
        },
    });
    ...
    return (
        <Card>
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
                <div>
                    <h3 className="text-[15px] font-semibold text-fg">Take student attendance</h3>
                    <p className="text-[12.5px] text-muted">Select a class and date, then mark each student</p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    <select value={classConfigId} onChange={(e) => setClassConfigId(Number(e.target.value))} className={selCls}>
            ...
            <div className="overflow-x-auto border-t border-border">
                {!classConfigId ? (
                    <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
                        <p className="text-[14px] font-medium text-fg">Select a class to take attendance</p>
                    </div>
                ) : isLoading ? (
                    <div className="flex items-center justify-center gap-2 py-16 text-muted"><Loader2 size={18} className="animate-spin" /> Loading…</div>
                ) : roster.length === 0 ? (
                    <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
                        <p className="text-[14px] font-medium text-fg">No students enrolled in this class</p>
                    </div>
                ) : (
                    <table className="w-full min-w-[720px] text-left text-[13.5px]">
                        ...
                        <tbody>
                            {roster.map((r) => {
                                const entry = entries[r.student_id];
                                if (!entry) return null;
                                return (
                                    <tr key={r.student_id} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                                                    ...
                                                    <button
                                                        ...
                                                        className={cn(
                                                            ...
                                                            entry.status === s.value
                                                                ? statusTone[s.value]
                                                                : 'text-muted ring-border hover:bg-surface-2',
                                                        )}
                                                    >
                                                        {s.label}
                                                    </button>
