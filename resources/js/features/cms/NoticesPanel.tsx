...
import { Plus, Pencil, Trash2, Star, Search, Paperclip, RotateCcw, ArrowLeft } from 'lucide-react';
import { Card, Button, Badge } from '@/components/ui';
import { ConfirmDialog } from '@/components/Modal';
import { toApiError } from '@/lib/api';
import { useToast } from '@/components/Toast';
import { listNotices, deleteNotice, restoreNotice, forceDeleteNotice, inputCls, type NoticeRow } from './api';
...
export function NoticesPanel() {
    ...
    const [forceDeleting, setForceDeleting] = useState<NoticeRow | null>(null);
    ...
    const restore = useMutation({
        mutationFn: (id: number) => restoreNotice(id),
        onSuccess: () => {
            invalidate();
    ...
    const forceDel = useMutation({
        mutationFn: (id: number) => forceDeleteNotice(id),
        onSuccess: () => {
            invalidate();
        ...
        <Card>
            <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-[15px] font-semibold text-fg">{isTrashedView ? 'Trashed Notices' : 'Notices'}</h3>
            ...
            <div className="overflow-x-auto border-t border-border">
                {isLoading ? <Loading /> : rows.length === 0 ? <Empty label={search ? 'No notices match your search' : isTrashedView ? 'Trash is empty' : 'No notices yet'} /> : (
            ...
            <ConfirmDialog open={!!deleting} onClose={() => setDeleting(null)} onConfirm={() => deleting && del.mutate(deleting.id)} busy={del.isPending}
                title="Move notice to trash" message={`Move "${deleting?.title}" to trash? You can restore it later.`} />
            ...
            <ConfirmDialog open={!!forceDeleting} onClose={() => setForceDeleting(null)} onConfirm={() => forceDeleting && forceDel.mutate(forceDeleting.id)} busy={forceDel.isPending}
                title="Permanently delete notice" message={`Permanently delete "${forceDeleting?.title}"? This action cannot be undone.`} />
        </Card>
    );
}
