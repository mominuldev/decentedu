...
import { listTemplates, createTemplate, updateTemplate, deleteTemplate, type TemplateRow } from './api';

const TYPES: { value: TemplateRow['type']; label: string }[] = [
...
export function TemplatesPanel() {
    ...
    const { data: rows = [], isLoading } = useQuery({ queryKey: ['sms-templates'], queryFn: listTemplates });
    const [editing, setEditing] = useState<TemplateRow | null>(null);
    const [creating, setCreating] = useState(false);
    const [deleting, setDeleting] = useState<TemplateRow | null>(null);

    const invalidate = () => qc.invalidateQueries({ queryKey: ['sms-templates'] });
    ...
    return (
        <Card>
            <div className="flex items-center justify-between px-5 py-4">
                <div>
                    <h3 className="text-[15px] font-semibold text-fg">SMS templates</h3>
                    <p className="text-[12.5px] text-muted">{rows.length} template{rows.length === 1 ? '' : 's'}</p>
                </div>
                        ...
                        <tbody>
                            {rows.map((r) => (
                                <tr key={r.id} className="border-b border-border last:border-0 hover:bg-surface-2/50">
                                    ...
                                    <td className="max-w-[320px] truncate px-5 py-3 text-muted">{r.message}</td>
            ...
            {(creating || editing) && (
                <TemplateForm row={editing} onClose={() => { setCreating(false); setEditing(null); }} onSaved={() => { invalidate(); setCreating(false); setEditing(null); }} />
            )}
...
function TemplateForm({ row, onClose, onSaved }: { row: TemplateRow | null; onClose: () => void; onSaved: () => void }) {
    const [form, setForm] = useState({
        name: row?.name ?? '', type: row?.type ?? 'general', message: row?.message ?? '', status: row?.status ?? true,
    });
    const [errors, setErrors] = useState<Record<string, string[]>>({});
    const [error, setError] = useState<string | null>(null);

    const save = useMutation({
        mutationFn: () => (row ? updateTemplate(row.id, form) : createTemplate(form)),
        onSuccess: onSaved,
        ...
        >
            ...
            <div className="space-y-4">
                ...
                <div>
                    ...
                    <select value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value as TemplateRow['type'] }))} className={inputCls(!!errors.type)}>
                ...
                <div>
                    <label className="mb-1.5 block text-[13px] font-medium text-fg">
                        ...
                        <span className="ml-1 font-normal text-faint">— use {'{student_name}'} / {'{date}'} for attendance templates</span>
                    </label>
                    <textarea rows={3} value={form.message} onChange={(e) => setForm((f) => ({ ...f, message: e.target.value }))} className={inputCls(!!errors.message)} />
                    {errors.message && <p className="mt-1.5 text-[12px] text-rose-500">{errors.message[0]}</p>}
                </div>
                <div>
