...
import { Plus, Loader2, Inbox, Printer } from 'lucide-react';
import { Card, Button } from '@/components/ui';
import { Modal } from '@/components/Modal';
import { toApiError } from '@/lib/api';
import { listClassConfigs, listSetup } from '@/features/academic/api';
import { listStudents } from '@/features/students/api';
import { listTransferCertificates, issueTransferCertificate, getTransferCertificate } from './api';
...
export function TransferCertificatesPanel() {
    const qc = useQueryClient();
    const { data: rows = [], isLoading } = useQuery({ queryKey: ['transfer-certificates'], queryFn: () => listTransferCertificates() });
    ...
    return (
        <Card>
            <div className="flex items-center justify-between px-5 py-4">
                <div>
                    <h3 className="text-[15px] font-semibold text-fg">Transfer certificates</h3>
                    <p className="text-[12.5px] text-muted">Issuing a TC marks the student as transferred.</p>
                </div>
                <Button onClick={() => setCreating(true)}><Plus size={16} /> Issue TC</Button>
            </div>
            ...
            <div className="overflow-x-auto border-t border-border">
                {isLoading ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
                        ...
                        <p className="text-[14px] font-medium text-fg">No transfer certificates issued yet</p>
                    </div>
                ) : (
                    <table className="w-full min-w-[560px] text-left text-[13.5px]">
                        <thead>
                            <tr className="border-b border-border text-[11px] uppercase tracking-wide text-faint">
                                <th className="px-5 py-2.5 font-semibold">TC No.</th>
            ...
            {creating && <IssueForm onClose={() => setCreating(false)} onSaved={() => { qc.invalidateQueries({ queryKey: ['transfer-certificates'] }); setCreating(false); }} />}
            {printing !== null && <TcPrintView id={printing} onClose={() => setPrinting(null)} />}
        </Card>
    );
}

function TcPrintView({ id, onClose }: { id: number; onClose: () => void }) {
    const { data } = useQuery({ queryKey: ['transfer-certificate', id], queryFn: () => getTransferCertificate(id) });
    if (!data) return null;
    const { certificate: c, branch, signatures } = data;

    return (
        <PrintableDocument title={`Transfer Certificate — ${c.certificate_number}`} onClose={onClose}>
            <div className="text-center">
                <h1 className="text-2xl font-bold">{branch.name}</h1>
                {branch.address && <p className="text-sm">{branch.address}</p>}
                <h2 className="mt-6 text-lg font-semibold underline">Transfer Certificate</h2>
                <p className="mt-1 text-sm">No: {c.certificate_number}</p>
            </div>
            <div className="mt-8 space-y-3 text-[15px] leading-relaxed">
                <p>This is to certify that <strong>{c.student.name}</strong> (Student ID: {c.student.student_uid}), son/daughter of <strong>{c.student.fathers_name}</strong>, was a student of this institution.</p>
                {c.reason_for_leaving && <p>Reason for leaving: {c.reason_for_leaving}</p>}
...
function IssueForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
    ...
    const [form, setForm] = useState({
        student_id: '', issue_date: new Date().toISOString().slice(0, 10), reason_for_leaving: '', remarks: '',
    ...
    const save = useMutation({
        mutationFn: () => issueTransferCertificate({
            ...form,
    ...
    return (
        <Modal
            open onClose={onClose} title="Issue transfer certificate"
            footer={<>
                <Button variant="outline" onClick={onClose} disabled={save.isPending}>Cancel</Button>
                <Button onClick={() => { setError(null); setErrors({}); save.mutate(); }} disabled={save.isPending}>
                    {save.isPending ? <Loader2 size={16} className="animate-spin" /> : null} Issue TC
                </Button>
            </>}
        >
            ...
            <div className="space-y-4">
                ...
                <div>
                    <label className="mb-1.5 block text-[13px] font-medium text-fg">Reason for leaving</label>
                    <textarea rows={2} value={form.reason_for_leaving} onChange={(e) => set('reason_for_leaving', e.target.value)} className={inputCls} />
                </div>
                <div>
